...
class MainBattle():
    def __init__(self):
        ...
        self.main_feedback = Label(self.main_frame, textvariable=feedback, font=("Gothic", "15", "bold"))
...
class Attack:
    ...
    def confirm_answer(self):
        ...
        if response == quiz[f"question{counter-1}"]["answer"]:
            self.feedback.config(text="Good job! Now close this window using the cross on the top right")
            self.confirm_button.config(state=DISABLED)
            feedback.set("Well done")
