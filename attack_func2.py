...
class Attack:
    ...
    def confirm_answer(self):
        '''program checks the user's answer to the answer of the quiz and bring the 
        user back to main battle window and tell them if they got it right or not'''
        ...
        print(response)
        print(quiz[f"question{counter-1}"]["answer"])

        if response == quiz[f"question{counter-1}"]["answer"]: 

            self.feedback.config(text="Good job!")
        
        else:
            self.feedback.config(text="Wrong answer")
