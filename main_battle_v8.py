...
class MainBattle():
    ...
    def __init__(self):
        ...
        #self.music = winsound.PlaySound("phasesong.wav", winsound.SND_ASYNC) # if we don't have async then any code after the music will not be played. # it is similar to threading. 
...
class Attack:
    ...
    def change_background(self):
        ...
        elif background_counter == 1: # once background counter has gone up, everything in the program will be changed into phase 2 colours
        ...
        else: # when the background increase again, everything in the program will be changed into phase 3 colours
        ...
        if response not in ["1", "2", "3", "4"]:
            ...
            if response == quiz[f"question{counter-1}"]["answer"]: # if the response is the same as the answer then the player does damage to the boss
                ...
                if BOSS_PERCENTAGE > 50:
                    ...
                    self.partner.main_feedback.config(text="Correct answer! You did 10 damage to Eason!") # updating and showing the new boss HP
                ...
                elif BOSS_PERCENTAGE <= 50 and BOSS_PERCENTAGE > 30: # when the boss reach between 50 and 25 HP, the phase changes
                    background_counter = 1
                    self.change_background() # change phase
                    self.attack_quiz.destroy()
                    self.restore_button()
                    self.partner.main_feedback.config(text="Correct answer! You did 10 damage to Eason!")
                    self.partner.boss_health.config(text=f"♥ ▬▬▬▬{BOSS_PERCENTAGE-10}%▬▬▬▬")
                    BOSS_PERCENTAGE -= 10
