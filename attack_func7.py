...
program also does boundary input on the numbers inputted. 

version 6: The Health bar has been added. The health bars decrease base on the user's answers

version 7: When the player or the boss's health reaches 0, the quiz window will close, while the main window congragulate the user if they win, 
and say try again to the user if they lose. The attack button will also be disabled, this way the user has to be close the window
since there is nothing to do. And most importantly, the program now follows my initial plans. As soon as the player clicks on the confirm button
the quiz window closes while the text in the main window updates. This makes the GUI much easier to use and more user friendly. This is 
all thanks to me fixing the .partner function. '''
...
class Attack:
    ...
    def confirm_answer(self):
        ...
        if response not in ["1", "2", "3", "4"]:
            ...
            if response == quiz[f"question{counter-1}"]["answer"]: # if the user gets it correct, the text in main window changes as well as the HP of boss
                ...
                if BOSS_PERCENTAGE == 0: # when the boss reaches 0% hp, the quiz window closes and disables the attack button
                    self.attack_quiz.destroy()
                ...
                if PLAYER_PERCENTAGE == 0: # when the player reaches 0% hp, the quiz window closes and disables the attack button
                    self.attack_quiz.destroy()
