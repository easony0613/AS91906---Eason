...
PLAYER_PERCENTAGE = 100 # setting up the variable to keep track of the boss health percentage
...
class MainBattle():
    ...
    def __init__(self):
        ...
        # feedback message
        self.main_feedback = Label(self.battle_frame, text=mood, fg="red", bg="#C3B1E1", font=text_font, wraplength=300)
...
class Attack:
    ...
    def __init__(self, partner):
        self.attack_quiz = Toplevel() # Making a new window on top of a existing window
        background = "#221B27"
    ...
    def close_attack(self, partner):
        '''put attack button back to normal'''
        # disables the buttons and not allow the user to do anything other than closing the program
        partner.attack_button.config(state=DISABLED)
        partner.help_button.config(state=DISABLED)
    ...
    def confirm_answer(self):
        ...
        if response not in ["1", "2", "3", "4"]:
            ...
            if response == quiz[f"question{counter-1}"]["answer"]: # if the response is the same as the answer then the player does damage to the boss
                ...
                self.attack_quiz.destroy() # the window will immediately close
                self.partner.attack_button.config(state=NORMAL) # the buttons in the main program will be back to normal
                ...
                self.partner.boss_health.config(text=f"♥ ▬▬▬▬{BOSS_PERCENTAGE-5}%▬▬▬▬") # updating and showing the new boss HP
                BOSS_PERCENTAGE -= 5 
            ...
            else: # if the player's response is not the answer
                global PLAYER_PERCENTAGE # accessing the player's hp
                self.attack_quiz.destroy() # closes the window immediately
                self.partner.attack_button.config(state=NORMAL) # turn both buttons in the main program back to normal
                self.partner.help_button.config(state=NORMAL)
                self.partner.main_feedback.config(text="Wrong answer, Eason did 20 damage to you!") # updating the feedback
                self.partner.player_health.config(text=f"♥ ▬▬▬▬{PLAYER_PERCENTAGE-20}%▬▬▬▬") # update the player's health bar
                PLAYER_PERCENTAGE -= 20

                if PLAYER_PERCENTAGE == 0: # if the user's hp reaches 0%
                    self.attack_quiz.destroy() # the window closes
                    self.partner.main_feedback.config(text="You lose") # update and text and disable both buttons
                    self.partner.attack_button.config(state=DISABLED)
                    self.partner.help_button.config(state=DISABLED)
