...
program also does boundary testing on the numbers inputted. The quiz window immediately closes as the confirm button is pressed'''
...
class Attack:
    def __init__(self, partner):
        ...
        # the button for user to confirm and submit their answer
    ...
    def confirm_answer(self):
        ...
        if response not in ["1", "2", "3", "4"]:
            self.feedback.config(text="Please enter the number from the choice given above")
            response = self.answer_entry.get()
            ...
            if response == quiz[f"question{counter-1}"]["answer"]:
                ...
                self.partner.main_feedback.config(text="I weeeeeeeee")

            else:
                self.feedback.config(text="Wrong answer! Now close this window using the cross on the top right")
                self.attack_quiz.destroy()
                self.partner.attack_button.config(state=NORMAL)
                self.partner.main_feedback.config(text="I dieeeeeeee")
